...
PAYLOADS_DIR="$PORTAL_DIR/payloads"
...
cleanup() {
    ...
    python3 "$LIB_DIR/network/hosts_manager.py" remove 2>/dev/null || true
...
python3 "$LIB_DIR/network/hosts_manager.py" add || die "Failed to modify /etc/hosts"
...
python3 "$LIB_DIR/http/portal_https.py" &
SERVER_PID=$!
...
python3 "$LIB_DIR/usb/usb_bypass.py"
BYPASS_EXIT=$?
...
if [ "$BYPASS_EXIT" -eq 0 ]; then
    ...
    log "Check $LIB_DIR/http/access.log for request details (created at runtime)"
    log "Check $LIB_DIR/http/requests/ for captured activation data (created at runtime)"
fi
